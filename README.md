...
#### Getting started
The shift hours are from 2:30 to 10:30 pm, but those times are squishy. Be at the office no later than 3:30 pm. Generally it's worthwhile to check slack once more after you've commuted home.

**First things first:** When you get into the office, it's a good idea to announce yourself on Slack so folks know who is on duty. In `booking`, `asec-changes` and `mpeds` drop a line saying you're in office and you're on the night shift. Do this _especially_ if you're filling in for someone.

*Note:* `#asec-changes` is only operating M-F. The weekends always use `booking`.
...
Either the editors or the person on the day shift will know what's running in the paper the next day. Be aware that booking for the paper changes often.
...
**You're representing graphics.** On weekdays, the booking meetings are at 9 am and 4 pm. The editors usually go to these. If it's the weekend, there is the 1 pm and 5 pm meeting. Bonus: if it's a holiday, the morning meeting is at 11 am. Tim Curran (or Ed Thiede) runs the meeting. Everyone is asked how they like the front page and walked through the other section fronts. People will generally just agree that it looks good. Then, at the end, they'll go through things they're thinking for Monday's paper. Take notes. Ask about graphics if you need to.
...
1. [Methode](https://methode.washpost.com/vpn/index.html): Dense, but the best place to start.
    ...
    * **For more about Methode** go to the [Methode.md](/Methode.md) file.
2. `#booking`, `#asec-changes` and `#mpeds` channels in slack: These channels will be your best friends on the night shift, and they often overlap with the same content. Booking and Asec-Changes are run by the editors and designers. Look for the print booking for the day. It'll look something like this:
...
3. Check the print layout to make sure **1.** the graphic isn't cut off and **2.** the graphic is in the right color scale. Same way as before, `File > Quick Open > WP-YYYYMMDD-Daily.prp` etc. When you look at the page it's running on from the overview, you can see little boxes that'll indicate whether it's a color or black and white page. If it's in color, you're good. If it's in black and white, you might have to bring it into illustrator and convert it. To quickly do so, go to `Edit > Edit colors > Convert to greyscale`. You might have to tweak it to make sure it looks good and the colors are differentiated enough.
...
Don't worry. Check in with the designer first. Find them in `#booking`, `#asec-changes` or look in [Skedder](https://newsplan.washpost.com/skedder/) for who is assigned to the section. All else fails, just ask in Slack.
...
**The Twitter?! (Again.)** Go ahead and queue tweets for the next 22 or so hours. That way the Twitter won't sorely lapse. Projects *should* have tweets attached to their Trello cards. Schedule a tweet for every few hours (ignoring the hours between 10pm and 7am). That's about four or so tweets optimized for 6-8pm, 8-10pm, 8-10am, 11am-1pm. That gives the person on the next night shift time to do a similar queue schedule. It's perfectly fine if we don't have enough going on to make the Twitter that active.
...
3. If it's Saturday, send a weekend pass off note to the person working the weekend day shift, the person working the Sunday night shift (usually Aaron Steckelberg) and NEWS - Infographics Editors <graphicseds@washpost.com>. Super simple. Here's about what it should look like:
...
Head home and, if you feel so inclined, take one more look at Slack for any last minute requests.

**When do I fully sign off?** 10:30/11pm is generally when you can head out (sometimes even earlier). It's nice to commute home then take a look at slack one more time before you fully sign off to make sure nothing's exploded. Then finally, call it a night.
